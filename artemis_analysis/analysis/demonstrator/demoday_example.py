...
# Cpp tools
# from artemis_cpptools.tools.filtercoltool import FilterColTool
...
from cronus.dq.plotlytool import PlotlyTool

# Artemis core framework -- scheduling and IO
...
def example_configuration(table_id, seed=42):
    ...
    writer = BufferOutputWriter(
        "bufferwriter", BUFFER_MAX_SIZE=max_buffer_size, write_csv=write_csv
    )
...
class ExampleMenu(MenuBuilder):
    ...
    def _algo_builder(self):
        ...
        self._algos["testalgo"] = DummyAlgo1(
            "dummy", myproperty="ptest", loglevel="WARNING"
        )
        self._algos["csvalgo"] = CsvParserAlgo("csvparser", loglevel="WARNING")
        #        self._algos["filteralgo"] = FilterAlgo("filter", loglevel="WARNING")
        self._algos["profileralgo"] = ProfilerAlgo("profiler", loglevel="WARNING")

    def _seq_builder(self):
        # Define the sequences and node names
        self._seqs["seqX"] = Node(["initial"], ("csvparser",), "seqX")
        #        self._seqs["seqY"] = Node(["seqX"], ("filter",), "seqY")
        self._seqs["seqA"] = Node(["seqX"], ("profiler"), "seqA")
        self._seqs["seqB"] = Node(["seqY"], ("dummy"), "seqB")
